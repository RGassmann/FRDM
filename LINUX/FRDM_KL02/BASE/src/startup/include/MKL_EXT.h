...
#define __INIT_OUTPUT(Port, PinNr, Inv)            {((PORT_Type *)(PORTA_BASE + Port*(PORTB_BASE-PORTA_BASE)))->PCR[PinNr] = PORT_PCR_MUX(1); ((GPIO_Type *)(GPIOA_BASE+Port*(GPIOB_BASE-GPIOA_BASE)))->PDDR |= 1 << PinNr;}
...
#define __INIT_OUTPUT_POWER(Port, PinNr, InvMask)       {((PORT_Type *)(PORTA_BASE+Port*0x00001000))->PCR[PinNr] = PORT_PCR_MUX(1) | PORT_PCR_DSE_MASK;\
                                                         ((GPIO_Type *)(GPIOA_BASE+Port*0x040))->PDDR |= 1 << PinNr;}
...
#define     __OUTPUT_TOGGLE(Port, PinNr, Inv) \
            {((GPIO_Type *)(GPIOA_BASE+Port*0x040))->PTOR = 1 << PinNr;}
#define     OUTPUT_TOGGLE(Port_Name) \
            __OUTPUT_TOGGLE(Port_Name)

#define     __OUTPUT_SET(Port, PinNr, Inv, OnOff) \
            {((GPIO_Type *)(GPIOA_BASE+Port*0x040))->PCOR = ((!(OnOff ^ Inv)) & 1) << PinNr;\
                ((GPIO_Type *)(GPIOA_BASE+Port*0x040))->PSOR = ((OnOff ^ Inv) & 1) << PinNr;}
#define     OUTPUT_SET(Port_Name, OnOff) \
            __OUTPUT_SET(Port_Name, OnOff)

#define     __INPUT_GET(Port, PinNr, Inv) \
            ((((GPIO_Type *)(GPIOA_BASE+Port*0x040))->PDIR) >> PinNr  ^Inv) & 1
