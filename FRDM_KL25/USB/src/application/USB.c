...
static const uint8_t sd1[] = "GMN";                        // Manufacturer
static const uint8_t sd2[] = ProductDescription;           // Product Description
static const uint8_t sd3[] = SERIAL_NO;                    // Serial Number
static const uint8_t sd4[] = "PCS-MASTER Interface";        // Interface Association #1
static const uint8_t sd5[] = "Interface 0 - PCSM";        // Interface #0
static const uint8_t sd6[] = "PCS CDC Interface";        // Interface Association #2
...
static void ep5StartTxTransactionIfIdle() {
   ...
   if ((epHardwareState[5].state == EPIdle) && (cdc_rxBufferItemCount()>0)) {
      ...
      //DEBUG_PIN_PCOR = DEBUG_PIN_MASK;/ROG
#endif
      ep5InitialiseBdtTx();
      epHardwareState[5].state = EPDataIn;
      serialDelayCount = 0;
