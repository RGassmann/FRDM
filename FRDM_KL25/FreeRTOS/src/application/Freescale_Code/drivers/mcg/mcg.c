...
int pll_init(int crystal_val, unsigned char hgo_val, unsigned char erefs_val, signed char prdiv_val, signed char vdiv_val, unsigned char mcgout_select)
{
  ...
  // The loss of lock interrupt can be enabled by seperately OR'ing in the LOLIE bit in MCG->C6
...
/*********************************************************************************************/
/* Functon name : new_pll_freq
 *
 * Mode transition: Moves from PEE to PBE mode before changing PLL settings and moves back to PEE mode
 *
 * This function provides a safe means of updating the PLL configurations to ensure the system
 * does not lose the clock source or is over-clocked as the PLL frequency changes.
 * Using the function parameter names the PLL frequency is calculated as follows:
 * PLL freq = ((crystal_val / prdiv_val) * vdiv_val)
 * The various passed parameters are checked to ensure they are within the allowed range. If any
 * of these checks fail the driver will exit and return a fail/error code. An error code will
 * also be returned if any error occurs during the PLL initialization sequence. Refer to the
 * readme file in the mcg driver directory for a list of all these codes.
 * This function first moves the MCG to PBE mode to use the external reference clock to provide the
 * system clock. The PLL settings are then updated. Once the PLL has re-locked the MCG is moved back
 * to PEE to use the PLL as the system clock.
 * Only the PLL PRDIV0 and VDIV0 fields are changed.
 *
 * Prerequisistes : The MCG must be in PEE mode before calling this routine. It is the responsibility
 * of the calling routine to ensure the SIM->CLKDIV1 system clock dividers and any peripheral clock
 * dividers are set correctly before or after calling this function to ensure the appropriate clocks
 * are kept within specification. You would typically change the divider values before calling this
 * routine.
 *
 * Parameters: crystal_val - external clock frequency in Hz either from a crystal or square
 *			     wave clock source
...
int pbe_pee(int crystal_val)
{
  ...
  if (!((((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x2) && // check CLKS mux has selcted external reference
      (!(MCG->S & MCG_S_IREFST_MASK)) &&			       // check FLL ref is external ref clk
      (MCG->S & MCG_S_PLLST_MASK) &&				       // check PLLS mux has selected PLL
      (!(MCG->C2 & MCG_C2_LP_MASK))))				       // check MCG->C2[LP] bit is not set
  {
...
// Check MCG is in BLPE mode
  if (!((((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x2) && // check CLKS mux has selcted external reference
      (!(MCG->S & MCG_S_IREFST_MASK)) &&			       // check FLL ref is external ref clk
      (MCG->C2 & MCG_C2_LP_MASK)))				       // check MCG->C2[LP] bit is set
  {
 ...
 * achieved by setting the MCG->C2[LP] bit. There is no status bit to
 ...
 * achieved by clearing the MCG->C2[LP] bit. There is no status bit to
 * check so 0 is always returned if the function was called with the MCG
 * in BLPI mode.
 *
 * Parameters: irc_freq - internal reference clock frequency
...
unsigned char atc(unsigned char irc_select, int irc_freq, int mcg_out_freq)
{
  ...
  bus_clock_freq = mcg_out_freq / (((SIM->CLKDIV1) >> 16)+ 1);//For KL25, flash and bus use the same bus div
  if ((bus_clock_freq < 8000000) || (bus_clock_freq > 16000000))
  {
    SIM->CLKDIV1 = orig_div; // set SIM->CLKDIV1 back to original value
 ...
 * achieved by setting or clearing the MCG->C6[CME] bit. It is recommended to
...
/********************************************************************/
/* Functon name : chk_for_resistor
 *
 * This function is specifically for the Freescale Tower and Freedom boards.
 * It performs a simple check to determine if there is an 1M ohm external
 * feedback resistor connected between extal and xtal. It returns a 1 if the
 * resistor is detected or a 0 if the resistor is not detected. This can be used
 * by the calling routine to compare the test result with the value for HGO that
 * is being used to configure the oscillator. This helps ensure the correct
 * configuration is used.
 *
 * This check is used just to test for the presence of a 1M resistor, it MUST
 * NOT be used in user code to automatically configure the oscillator HGO value.
 *
 * It simply configures the crystal pins as GPIO outputs, sets them both low,
 * configures one as high and then configures the other as an input (no pull
 * up). If the resistor is present then the input will be pulled high. This
 * process is repeated for the pin functions reversed. Both conditions have
 * to result in the input being pulled high for the resistor to be identifiedas
 * being present.
 */
unsigned char chk_for_resistor(void)
{
  ...
  FPTA->PCOR = 0xC0000; // clear PTA18 and 19 output data
  FPTA->PDDR |= 0xC0000; // set PTA18 and 19 as outputs
  PORTA->GPCHR = (0xC0000 | PORT_PCR_MUX(1)); // configure the pin muxes for GPIO
  // Drive EXTAL high
  FPTA->PSOR = 0x40000; // set PTA18 data out high
  // Configure XTAL as an Input, no pull up
  FPTA->PDDR &= ~(0x80000); // clear PTA19 data direction to make it an input
  // Wait for ~2 time constants
  for (i = 0 ; i < 300 ; i++) {}
  // Check if XTAL was pulled high
  if (FPTA->PDIR & 0x80000)
  {
    xtal_high = 1;
  }
  // Configure EXTAL and XTAL as GPIO driving low
  FPTA->PCOR = 0xC0000; // clear PTA18 and 19 data output
  FPTA->PDDR |= 0xC0000; // set PTA18 and 19 as outputs
  // Drive XTAL high
  FPTA->PSOR = 0x80000; // set PTA19 data out high
  // Configure EXTAL as Input, no pull up
  FPTA->PDDR &= ~(0x40000); // clear PTA18 data direction to make it an input
  // Wait for ~2 time constants
  for (i = 0 ; i < 300 ; i++) {}
  // Check if EXTAL was pulled high
  if (FPTA->PDIR & 0x40000)
  {
    extal_high = 1;
  }

  // Now configure both pins back to their default state
  PORTA->GPCHR = 0x000C0000; // clear pta18 and 19 mux values to default
  FPTA->PDDR &= ~(0xC0000);    // set PTA18 and 19 as inputs
  FPTA->PCOR = 0xC0000;	       // clear PTA18 and 19 output data
