...
void uart_init (UART_Type * uartch, int sysclk, int baud)
{
	...
	SIM->SCGC4 |= SIM_SCGC4_UART2_MASK;
      ...
      uartch->BDL = (uint8)(sbr & UART_BDL_SBR_MASK);
...
void uart0_init (UARTLP_Type * uartch, int sysclk, int baud)
{
	...
	UART0->BDL = (uint8)(sbr_val & UARTLP_BDL_SBR_MASK);
...
{
      /* Wait until character has been received */
      while (!(channel->S1 & UARTLP_S1_RDRF_MASK));
...
{
      /* Wait until space is available in the FIFO */
      while(!(channel->S1 & UARTLP_S1_TDRE_MASK));
...
{
    return (channel->S1 & UARTLP_S1_RDRF_MASK);
}
/********************************************************************/
