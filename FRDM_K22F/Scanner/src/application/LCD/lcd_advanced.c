...
void lcd_draw_bitmap(int16 x, int16 y, const LCD_BITMAP *bitmap)
{
    ...
    for(used=1; used<(bitmap->width*bitmap->height); used++) {
        ...
        if(bitmap->data[index] & mask) { // if pixel value is one
            writedata16_last(d_theme.Font_Color.Word);
        }
        else {                      // or pixel value is zero
            writedata16_last(d_theme.BG_Color.Word);
        }
...
{
    if(percentage==100) {
        lcd_draw_rect(x,x+12,y,y+8, ILI9341_Green, 1);      // cap
        lcd_draw_rect(x+13,x+14,y+2,y+6, ILI9341_Green, 1); // area
    }
    else {
        lcd_draw_rect(x+(percentage>>3),x+12,y,y+8, ILI9341_GRAY2, 1);   // void
        lcd_draw_rect(x+13,x+14,y+2,y+6, ILI9341_GRAY2, 1);              // cap
        if( percentage >= 30 )
            lcd_draw_rect(x,x+(percentage>>3),y,y+8, ILI9341_Green, 1);     // area
        else if( percentage >= 15 )
            lcd_draw_rect(x,x+(percentage>>3),y,y+8, ILI9341_Orange, 1);     // area
        else
            lcd_draw_rect(x,x+(percentage>>3),y,y+8, ILI9341_Red, 1);     // area
    }
