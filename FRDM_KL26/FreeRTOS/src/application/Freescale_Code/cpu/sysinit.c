...
void sys_init (void)
{
	...
	*/
	...
	periph_clk_khz = core_clk_khz / (((SIM->CLKDIV1 & SIM_CLKDIV1_OUTDIV4_MASK) >> 16)+ 1);
...
//	  enable_abort_button();
	...
	if (TERM_PORT_NUM == 0)
	{
	  ...
	  PORTA->PCR[14] = PORT_PCR_MUX(0x3); // UART0 is alt3 function for this pin

	  /* Enable the UART_RXD function on PTA15 */
	  PORTA->PCR[15] = PORT_PCR_MUX(0x3); // UART0 is alt3 function for this pin
#endif
	...
	if (TERM_PORT_NUM == 1)
	{
		...
		PORTC->PCR[4] = PORT_PCR_MUX(0x3); // SCI is alt3 function for this pin

		/* Enable the SCI1_RXD function on PTC3 */
		PORTC->PCR[3] = PORT_PCR_MUX(0x3); // SCI is alt3 function for this pin
	}

	if (TERM_PORT_NUM == 2)
	{
		 /* Enable the UART_TXD function on PTD3 */
		PORTD->PCR[3] = PORT_PCR_MUX(0x3); // UART2 is alt3 function for this pin

		/* Enable the UART_RXD function on PTD2 */
		PORTD->PCR[2] = PORT_PCR_MUX(0x3); // UART2 is alt3 function for this pin
	}
	 ...
	 */
	if (TERM_PORT_NUM == 0)
	    uart0_init (UART0, uart0_clk_khz, TERMINAL_BAUD);
	else if (TERM_PORT_NUM == 1)
	    uart_init (UART1, periph_clk_khz, TERMINAL_BAUD);
	else if (TERM_PORT_NUM == 2)
	    uart_init (UART2, periph_clk_khz, TERMINAL_BAUD);
	else
	  while(1);
}
/********************************************************************/
void enable_abort_button(void)
{
    ...
    PORTA->PCR[4] = PORT_PCR_MUX(0x1); // GPIO is alt1 function for this pin

    /* Configure the PTA4 pin for rising edge interrupts */
    PORTA->PCR[4] |= PORT_PCR_IRQC(0x9);
